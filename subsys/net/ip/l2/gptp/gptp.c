...
{
	struct device *dev = GPTP_PORT_DRV(port);
...
static bool gptp_handle_critical_msg(struct net_if *iface, struct net_pkt *pkt)
{
	...
	struct gptp_pdelay_resp_state *pdelay_resp_state;
	int port;
	bool handled = false;
	...
	case GPTP_PATH_DELAY_REQ_MESSAGE:
		...
		PRINT_INFO("PDELAY_REQUEST", hdr, pkt);

		if (pdelay_resp_state->state != GPTP_PDELAY_RESP_NOT_ENABLED) {
...
static void gptp_handle_msg(struct net_pkt *pkt)
{
	...
	case GPTP_PATH_DELAY_RESP_MESSAGE:
		...
		PRINT_INFO("PATH_DELAY_RESP", hdr, pkt);
	...
	case GPTP_FOLLOWUP_MESSAGE:
		...
		NET_DBG("Keeping follow-up seq %d msg %p", hdr->sequence_id,
			pkt);
		break;
	...
	case GPTP_PATH_DELAY_FOLLOWUP_MESSAGE:
		...
		PRINT_INFO("PATH_DELAY_FOLLOWUP", hdr, pkt);
...
enum net_verdict net_gptp_recv(struct net_if *iface, struct net_pkt *pkt)
{
	...
	if ((hdr->ptp_version != GPTP_VERSION) ||
		...
		/* The Stack only supports PTP V2 and transportSpecific set to 1
		 * with IEEE802.1AS-2011.
		 */
		goto drop;
	}
...
static void gptp_init_clock_ds(void)
{
	...
	memcpy(&parent_ds->port_id.clk_id,
	       &default_ds->clk_id, GPTP_CLOCK_ID_LEN);
	memcpy(&parent_ds->gm_id,
	       &default_ds->clk_id, GPTP_CLOCK_ID_LEN);
...
static void gptp_init_port_ds(int port)
{
	...
	/* Substract 1 to divide by 2 the sync interval. */
...
static void gptp_add_port(struct net_if *iface, void *user_data)
{
	...
	clk = ptp_clock_lookup_by_dev(net_if_get_device(iface));
	if (clk) {
