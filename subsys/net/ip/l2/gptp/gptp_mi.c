...
static void gptp_mi_half_sync_itv_timeout(struct k_timer *timer)
{
	...
	for (port = GPTP_PORT_START; port < GPTP_PORT_END; port++) {
		...
		if (timer == &state->half_sync_itv_timer) {
...
static void gptp_mi_sync_receipt_timeout(struct k_timer *timer)
{
	struct gptp_pss_send_state *state_send;
	struct gptp_pss_rcv_state *state_rcv;
	int port;

	for (port = GPTP_PORT_START; port < GPTP_PORT_END; port++) {
		state_send = &GPTP_PORT_STATE(port)->pss_send;
		if (timer == &state_send->sync_receipt_timeout_timer) {
			state_send->sync_receipt_timeout_timer_expired = true;
		}

		state_rcv = &GPTP_PORT_STATE(port)->pss_rcv;
		if (timer == &state_rcv->sync_receipt_timeout_timer) {
			state_rcv->sync_receipt_timeout_timer_expired = true;
		}
...
static void gptp_mi_init_port_sync_sync_rcv_sm(int port)
{
	...
	k_timer_init(&pss_rcv->sync_receipt_timeout_timer,
		     gptp_mi_sync_receipt_timeout, NULL);
}
...
static void gptp_mi_init_port_sync_sync_send_sm(int port)
{
	...
	k_timer_init(&pss_send->sync_receipt_timeout_timer,
		     gptp_mi_sync_receipt_timeout, NULL);
...
static void announce_timer_handler(struct k_timer *timer)
{
	...
	for (port = GPTP_PORT_START; port < GPTP_PORT_END; port++) {
		...
		if (timer == &state->ann_rcpt_expiry_timer) {
...
static void announce_periodic_timer_handler(struct k_timer *timer)
{
	...
	for (port = GPTP_PORT_START; port < GPTP_PORT_END; port++) {
		...
		if (timer == &state->ann_send_periodic_timer) {
...
u64_t gptp_get_current_time_nanosecond(int port)
{
	...
	clk = ptp_clock_lookup_by_dev(GPTP_PORT_DRV(port));
	if (clk) {
...
static void gptp_mi_pss_rcv_state_machine(int port)
{
	...
	case GPTP_PSS_RCV_DISCARD:
		k_timer_stop(&state->sync_receipt_timeout_timer);
		state->sync_receipt_timeout_timer_expired = false;
...
static void gptp_mi_pss_send_state_machine(int port)
{
	...
	case GPTP_PSS_SEND_SET_SYNC_RECEIPT_TIMEOUT:
		...
		if ((global_ds->selected_role[port] == GPTP_PORT_MASTER) &&
			...
			/* Change state as it may have transitionned from
			 * SEND_MD_SYNC.
			 */
			...
			k_timer_start(&state->sync_receipt_timeout_timer,
				      duration, 0);

		} else if (state->sync_receipt_timeout_timer_expired) {
			...
			state->state = GPTP_PA_INFO_RECEIVE;
			   ...
			   (state->ann_expired ||
			    (global_ds->gm_present &&
			     pss_rcv->sync_receipt_timeout_timer_expired))) {
			state->state = GPTP_PA_INFO_AGED;
		}
