        ...
        const u8_t *tx_data, int size)
{
    ...
    sys_write8(UART_EV_TX, UART_EV_PENDING);
...
static void liteuart_uart_irq_handler(void *arg)
{
    ...
    /* ack irq */
    pending = sys_read8(UART_EV_PENDING);
    sys_write8(sys_read8(UART_EV_PENDING), UART_EV_PENDING);
...
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
    IRQ_CONNECT(UART_IRQ, 100, liteuart_uart_irq_handler, DEVICE_GET(uart_liteuart_0), 0);
    irq_enable(UART_IRQ);
#endif
