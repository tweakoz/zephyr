...
static void queue0_isr(void *arg)
{
	...
	if (isr & GMAC_INT_RX_ERR_BITS) {
		...
		SYS_LOG_DBG("rx.w1=0x%08x, tail=%d",
			    rx_desc_list.buf[rx_desc_list.tail].w1,
			    rx_desc_list.tail);
		eth_rx(queue);
	}
...
#if defined(CONFIG_PTP_CLOCK_SAM_GMAC)
static int ptp_clock_sam_gmac_set(struct ptp_clock *clk,
				  struct net_ptp_time *tm)
{
	const struct eth_sam_dev_cfg *const cfg = DEV_CFG(clk->dev);
	Gmac *gmac = cfg->regs;
...
static int ptp_clock_sam_gmac_get(struct ptp_clock *clk,
				  struct net_ptp_time *tm)
{
	const struct eth_sam_dev_cfg *const cfg = DEV_CFG(clk->dev);
	Gmac *gmac = cfg->regs;
...
static int ptp_clock_sam_gmac_adjust(struct ptp_clock *clk, int increment)
{
	const struct eth_sam_dev_cfg *const cfg = DEV_CFG(clk->dev);
	Gmac *gmac = cfg->regs;
...
static int ptp_clock_sam_gmac_rate_adjust(struct ptp_clock *clk, float ratio)
{
	const struct eth_sam_dev_cfg *const cfg = DEV_CFG(clk->dev);
	Gmac *gmac = cfg->regs;
	u8_t nanos;
...
PTP_CLOCK_DEVICE_INIT(eth0_sam_gmac, ptp_api);
#endif /* CONFIG_PTP_CLOCK_SAM_GMAC */
