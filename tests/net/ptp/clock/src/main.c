...
static int my_ptp_clock_set(struct ptp_clock *clk, struct net_ptp_time *tm)
{
	struct eth_context *ctx = clk->dev->driver_data;
...
static int my_ptp_clock_get(struct ptp_clock *clk, struct net_ptp_time *tm)
{
	struct eth_context *ctx = clk->dev->driver_data;
...
static int my_ptp_clock_adjust(struct ptp_clock *clk, int increment)
{
	struct eth_context *ctx = clk->dev->driver_data;
...
static int my_ptp_clock_rate_adjust(struct ptp_clock *clk, float ratio)
{
	return 0;
}
...
PTP_CLOCK_DEVICE_INIT(eth_test_1, api);
PTP_CLOCK_DEVICE_INIT(eth_test_2, api);
...
static void test_ptp_clock_interfaces(void)
{
	...
	idx = ptp_interface[0];
	clk = ptp_clock_lookup_by_dev(net_if_get_device(eth_interfaces[idx]));
	...
	clk = ptp_clock_lookup_by_dev(
		net_if_get_device(eth_interfaces[non_ptp_interface]));
